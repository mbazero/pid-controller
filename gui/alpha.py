...
class MainWindow(QWidget):
    def __init__(self, n_dac, n_dds):
        ...
        # HBox layout to hold global variables and channel tabs
        ...
        self.dac_chans = [ChannelControl(dac_array[count], pla) for count in range(n_dac)]
        self.dds_chans = [ChannelControl(dds_array[count], pla) for count in range(n_dds)]
    ...
    def indexToChan(self, index):
        ...
        if index >= self.n_dac :
            return self.dds_chans[index - self.n_dac]
        else :
...
class GlobalParams(QGroupBox):
    def __init__(self, pla, n_dac, n_dds):
        ...
        #################### num_locks #######################
        #self.locks_dec = range(1, self.n_out)
